...
export default function PoseStage({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (runningRef.current && lm) {
        drawSkeleton(ctx, lm, { width: w, height: h, mirror: true });
  ...
  return (
    <div className="relative aspect-[4/3] w-full border border-black/15 bg-white">
...
function idleStage(ctx: CanvasRenderingContext2D, w: number, h: number, running: boolean) {
  ...
  ctx.fillText(
    running ? "ACQUIRING POSE…" : "CAMERA IDLE · START SESSION TO BEGIN",
    w / 2,
    h / 2,
  );
}
