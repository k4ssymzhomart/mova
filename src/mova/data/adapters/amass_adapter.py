...
class AmassAdapter:
    def __init__(self, raw_dir: Path, out_dir: Path, seed: int, degrade: bool) -> None:
    ...
    def run(self) -> int:
        ...
        for path in files:
            ...
            # subject = top-level AMASS sub-dataset/subject dir; session = file stem
            subject_id = rel.parts[0] if len(rel.parts) > 1 else "amass"
...
def main(argv: list[str] | None = None) -> int:
    ...
    AmassAdapter(args.raw_dir, args.out_dir, seed=args.seed, degrade=not args.no_degrade).run()
    return 0
